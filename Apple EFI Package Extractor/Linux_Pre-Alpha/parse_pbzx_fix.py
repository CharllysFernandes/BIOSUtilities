...
#
# ---------------- Plato Mavropoulos @ 2021-11-05 ----------------
# Based on https://gist.github.com/Lekensteyn/6e0840e77bc9bd013f57,
# this version adds f_length 0xE90000 to data without compression
# and fixes non defind "offset" variable (should be in_offset).
# The script should be re-written from scratch with modern code.
...
def parse_pbzx(pbzx_file, cpio_file):
    ...
    while (flags & (1 << 24)):
        ...
        #with open('off.txt','a',encoding='utf-8') as o : o.write('0x%X' % f_length)
        if f_length in [0x1000000,0xE90000,0xF2EC00]:
            # Literal copy
            ...
            if xzmagic != b'\xfd7zXZ\x00':
                cpio_file.close()
                raise RuntimeError("Error: Header is not xar file header: offset %d, magic %r" % (in_offset, xzmagic))
            else:
                subprocess.run(['7z', 'x', '-aou', '-bso0', '-bse0', '-bsp0', '-o' + cpio_file + '_e', cpio_file], check=True)
                with open(cpio_file + '_e','wb') as f:
                    f.write(xzmagic)
                    # Do not copy header magic again (-6)
                    copy_data(pbzx_file, f, -6 + f_length)
